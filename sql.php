...
class sql extends dbconn {
	...
	public function saveNotif($msg,$time,$loop,$loop_every,$user,$title,$start,$end,$color){
		$db = $this->dblocal;
		try
		{
			$stmt = $db->prepare("insert into notif(notif_msg, notif_time, notif_repeat, notif_loop,username,title,start,end,color) values(:msg , :bctime , :repeat , :loop,:user,:title,:start,:end,:color) ");
